...
export async function searchCases(params: SearchCasesParams): Promise<SearchCasesResponse> {
  const { page, root, summaryItems } = await searchCaseSummaries(params);
...
async function searchCaseSummaries(params: SearchCasesParams) {
...
function getLawApiKey() {
  const key = process.env.LAW_API_KEY?.trim() || process.env.LAW_API_OC?.trim();

  if (!key) {
    throw new LawApiError("MISSING_API_KEY", "LAW_API_KEY 또는 LAW_API_OC가 설정되지 않았습니다.", 500);
  }
